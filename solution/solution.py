...
def run_assistant(
    ...
    while True:
        ...
        cost_stats = estimate_cost(user_msg, reply)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        g = r['gpt4o_answer'][:37] + "..." if len(r['gpt4o_answer']) > 40 else r['gpt4o_answer']
        m = r['mini_answer'][:37] + "..." if len(r['mini_answer']) > 40 else r['mini_answer']
        lines.append(f"{p:<40} | {g:<40} | {m:<40} | {r['gpt4o_time']:<15.2f} | {r['mini_time']:<15.2f}")
    return "\n".join(lines)
